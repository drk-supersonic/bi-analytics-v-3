...
if user is not None:
    ...
    with st.form("add_user_form"):
        ...
        with col2:
            ...
            new_role = st.selectbox("Роль *", options=list(ROLES.keys()), format_func=lambda x: ROLES[x])
        ...
        if submitted:
            if new_username and new_password:
                ...
                if create_user(new_username, new_password, new_role, new_email if new_email else None, user['username']):
                    st.success(f"✅ Пользователь {new_username} успешно создан!")
                    st.rerun()
                else:
                    st.error("❌ Ошибка при создании пользователя. Возможно, пользователь с таким именем уже существует.")
            else:
    ...
    if active_users:
        with st.form("change_role_form"):
            user_options = {f"{u[1]} ({get_user_role_display(u[2])})": u[0] for u in active_users}
            selected_user_display = st.selectbox("Выберите пользователя", options=list(user_options.keys()))
            ...
            if submitted:
                if new_role != current_role:
                    conn = sqlite3.connect(DB_PATH)
                    cursor = conn.cursor()
                    cursor.execute('UPDATE users SET role = ? WHERE id = ?', (new_role, selected_user_id))
                    conn.commit()
                    conn.close()
                    ...
                    st.success(f"✅ Роль пользователя {selected_username} успешно изменена на {get_user_role_display(new_role)}!")
                    st.rerun()
                else:
    ...
    if role_stats:
        role_data = [{'Роль': get_user_role_display(r[0]), 'Количество': r[1]} for r in role_stats]
    ...
    with st.form("settings_form"):
        ...
        if submitted:
            try:
                set_setting('finance_files_path', finance_path, SETTING_KEYS.get('finance_files_path'), user['username'])
                set_setting('plan_fact_files_path', plan_fact_path, SETTING_KEYS.get('plan_fact_files_path'), user['username'])
                set_setting('resources_files_path', resources_path, SETTING_KEYS.get('resources_files_path'), user['username'])
                
                log_action(user['username'], 'update_settings', 'Обновлены настройки путей к файлам')
...
# ==================== TAB 5: Обновление отчетов ====================
    ...
    with col1:
        if st.button("🔄 Обновить отчеты", type="primary", use_container_width=True):
            try:
                ...
                log_action(user['username'], 'force_update_reports', 'Принудительное обновление отчетов')
                st.success("✅ Отчеты успешно обновлены!")
                st.rerun()
            except Exception as e:
                log_action(user['username'], 'force_update_reports_error', f'Ошибка обновления отчетов: {str(e)}')
    ...
    if settings:
        ...
        st.warning("⚠️ Настройки путей к файлам не заданы. Перейдите во вкладку 'Настройки системы' для настройки.")
...
# ==================== TAB 6: Права доступа к проектам ====================
    ...
    with st.form("grant_permission_form"):
        ...
        with col2:
            project_name = st.text_input("Название проекта *", help="Введите название проекта")
        
        submitted = st.form_submit_button("Выдать права", type="primary")
        
        if submitted:
            if project_name:
                if grant_project_access(selected_user_id, project_name, user['username']):
                    log_action(
                        user['username'], 
                        'grant_project_access', 
                        f'Выданы права доступа пользователю {selected_user_display} к проекту {project_name}'
                    )
                    st.success(f"✅ Пользователю {selected_user_display} выданы права доступа к проекту {project_name}!")
                    st.rerun()
                else:
                    st.warning("⚠️ Права доступа уже существуют или произошла ошибка")
            else:
    ...
    if permissions:
        ...
        for project_name, project_perms in sorted(projects_dict.items()):
            with st.expander(f"📁 {project_name} ({len(project_perms)} пользователей)"):
                perms_data = []
                for perm in project_perms:
                    perms_data.append({
                        'Пользователь': perm['username'],
                        'Роль': get_user_role_display(perm['role']),
                        'Выдано': perm['granted_at'] if perm['granted_at'] else '-',
                        'Выдал': perm['granted_by'] or '-',
                        'Действие': f"revoke_{perm['user_id']}_{project_name}"
                    })
                
                df_perms = pd.DataFrame(perms_data)
                st.dataframe(df_perms[['Пользователь', 'Роль', 'Выдано', 'Выдал']], use_container_width=True, hide_index=True)
                    ...
                    with col2:
                        if st.button("Отозвать", key=f"revoke_{perm['id']}"):
                            if revoke_project_access(perm['user_id'], project_name):
                                ...
                                st.success(f"✅ Права доступа пользователя {perm['username']} к проекту {project_name} отозваны!")
                                st.rerun()
    else:
